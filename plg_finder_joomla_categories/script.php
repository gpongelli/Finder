 ...
 * @since       2.5
 */
class plgFinderJoomla_CategoriesInstallerScript
{
	/**
	...
	function activateButton()
	{
		...
		$query->where($db->quoteName('name').' = '.$db->quote('plg_finder_joomla_categories'));
		$db->setQuery($query);
		if (!$db->query())
		{
			JError::raiseNotice(1, JText::_('PLG_FINDER_JOOMLA_CATEGORIES_ERROR_ACTIVATING_PLUGIN'));
		}
	}
}
