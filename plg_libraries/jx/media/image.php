<?php
/**
 * @version		$Id: image.php 472 2009-09-27 18:19:30Z louis $
...
class JImage
{
	...
	function crop($width, $height, $left, $top, $createNew = true, $scaleMethod = JImage::SCALE_INSIDE)
	{
		...
		if ($createNew)
		{
			// Create the new JImage object for the new truecolor image handle.
	...
	function filter($type)
	{
		...
		$className = 'JImageFilter_'.ucfirst($name);
		if (!class_exists($className))
		{
			jimport('joomla.filesystem.path');
			if ($path = JPath::find(JImageFilter::addIncludePath(), strtolower($name).'.php'))
			{
				require_once $path;
	...
	function loadFromFile($path)
	{
		...
		$properties = JImageHelper::getProperties($path);
		if (!$properties) {
			return false;
		}
	...
	function _prepareDimensions($width, $height, $scaleMethod)
	{
		...
		$dimensions = array();
		if ($scaleMethod == JImage::SCALE_FILL)
		{
			$dimensions['width'] = $width;
			$dimensions['height'] = $height;
		}
		elseif ($scaleMethod == JImage::SCALE_INSIDE || $scaleMethod == JImage::SCALE_OUTSIDE)
		{
...
{
	/**
	 * Add a directory where JImage should search for filters. You may
