<?php
/**
 * @version		$Id: tablenested.php 498 2010-05-06 22:22:56Z robs $
...
class JTableNested extends JTable
{
	...
	public function check()
	{
		...
		{
			$this->_db->setQuery(
				'SELECT COUNT('.$this->_db->nameQuote($this->_tbl_key).')' .
				' FROM '.$this->_db->nameQuote($this->_tbl).
				' WHERE '.$this->_db->nameQuote($this->_tbl_key).' = '.$this->parent_id
			);
	...
	protected function _logtable($showData = true, $showQuery = true)
	{
		...
		if ($showData)
		{
			$key = $this->_tbl_key;
			$this->_db->setQuery(
				'SELECT `'.$key.'`, parent_id, lft, rgt, level' .
				' FROM `'.$this->_tbl.'`' .
				' ORDER BY `'.$key.'`'
			);
			$rows = (array) $this->_db->loadRowList();
