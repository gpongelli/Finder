...
class plgFinderJoomla_Newsfeeds extends FinderIndexerAdapter
{
	 ...
	 */
	public function onBeforeSaveJoomlaNewsfeed($id)
	{
	 ...
	 * @throws  Exception on database error.
	 */
	public function onChangeJoomlaNewsfeed($ids, $property, $value)
	{
		// Check if we are changing the news feed state.
		if ($property === 'published')
		{
			// The news feed published state is tied to the category published
	...
	public function onChangeJoomlaCategory($ids, $property, $value)
	{
		...
		if ($property === 'state')
		{
			// The newsfeed published state is tied to the category published
			// state so we need to look up all published states before we
			// change anything.
			foreach ($ids as $id)
			{
	 ...
	 * @throws  Exception on database error.
	 */
	public function onDeleteJoomlaNewsfeed($ids)
	{
		// Remove the items.
	...
	protected function getListQuery($sql = null)
	{
		...
		$sql->select('a.published AS state, a.ordering, a.publish_up AS start_date, a.params');
