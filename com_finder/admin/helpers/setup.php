...
class JXtendedSetupHelper
{
	...
	/**
	 * Setup the JXtended Libraries if not already setup.
	 *
	 * @return	boolean	True on success.
	 * @since	2.0
	 */
	public static function setupLibraries()
	{
		// Check to see if the libraries plugin files are present.
		if (!is_file(JPATH_ROOT.'/plugins/system/jx.php') || !is_dir(JPATH_ROOT.'/plugins/system/jx'))
		{
			/*
			 * TODO: TRY TO GET THE LIBRARIES FROM THE SERVER.
			 */

			/*
			 * Don't forget to import the libraries if gotten successfully.
			 */
		...
		if (empty($plugin))
		{
			...
			$plugin->name = JText::_('System - JXtended Libraries');
			...
			if (!$plugin->store())
			{
				JError::raiseWarning(500, JText::_('JX_Unable_To_Install_Plugin'));
				return false;
			}
		}
